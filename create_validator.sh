...
echo $balance $denom
sleep 2
while [[ `echo $balance` -lt  900000000 ]]
do
echo -e "\n=== Insufficient balance of tokens to create a validator. Tokens on balance $balance $denom, minimum 900000000$denom required ===="
echo -e "\n===================== Request tokens from faucet at ${address} ==========================="
echo -e "\n== Недостаточный баланс токенов для создания валидатора. Токенов на балансе $balance $denom, необходимо минимум 900000000$denom =="
echo -e "\n===================== Запросите токены из крана на адрес ${address} ==================="
sleep 1m
balance=`$binary q bank balances $address -o json | jq -r .balances[0].amount `
done
